...
def _init_rate_limiter():
    ...
    try:
        ...
        logfire.info("ðŸš¦ Rate limiting initialized via Redis.")
    except Exception as e:
        app.state.limiter = Limiter(key_func=get_remote_address)
        app.state.rate_limiter_storage = "memory"
        logfire.warning(f"âš ï¸ Redis unavailable ({e}); using in-memory rate limiting.")
...
def verify_api_key(credentials: HTTPAuthorizationCredentials = Depends(_security)):
    ...
    if not credentials or credentials.credentials != settings.API_KEY:
        logfire.warning("ðŸ”’ Unauthorized /query request: invalid or missing API key.")
        raise HTTPException(
...
def startup_event():
    ...
    if not settings.API_KEY:
        logfire.warning("ðŸ”“ RAG_API_KEY is not set â€” /query is open to anyone. Set it in production.")
...
def query(
    ...
    with logfire.span("ðŸ” /query", request_id=request_id, thread_id=thread_id):
        ...
        if rail_fired:
            ...
            logfire.info("ðŸ›¡ï¸ Request blocked by guardrails", request_id=request_id, thread_id=thread_id)
            return {
                "question": q,
