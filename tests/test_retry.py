...
def test_rerank_with_indices_retries_then_falls_back():
    """Indexed reranker should retry and fall back to input order."""
    docs = ["doc one", "doc two", "doc three"]

    with patch("app.services.retrieval.ranking_service._get_ranker") as mock_get_ranker:
        mock_ranker = MagicMock()
        mock_ranker.rerank_with_indices.side_effect = RuntimeError("transient")
        mock_get_ranker.return_value = mock_ranker

        results = rerank_with_indices("query", docs, top_n=2)
        ...
        assert mock_ranker.rerank_with_indices.call_count == 3
