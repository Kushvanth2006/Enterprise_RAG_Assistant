...
def test_check_neon_postgres_success():
    ...
    with patch(
        "app.services.health.connection_checker.psycopg.connect",
...
def test_check_gemini_gateway_success():
    mock_response = MagicMock()
    mock_response.content = "hi"

    with patch(
        "app.services.health.connection_checker.GeminiLLM.invoke",
        return_value=mock_response,
    ):
        result = _check_gemini_gateway()

    assert result.healthy is True
    assert result.name == "llm_gateway"


def test_check_gemini_gateway_empty_response():
    mock_response = MagicMock()
